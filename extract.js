...
async function extractFromDoc(doc) {
  ...
  try {
    ...
    // sometimes the model wraps the array in extra text, try to pull it out
    const match = raw.match(/\[[\s\S]*\]/);
    if (!match) throw new Error(`unparseable response for ${doc.id}`);
...
async function main() {
  ...
  for (const doc of docs) {
    ...
    try {
      const result = await extractFromDoc(doc);
